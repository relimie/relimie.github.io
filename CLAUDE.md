...
│   │   ├── articles/                    # Article-hub content (SEO): [slug]_[lang].md
...
### 5.1 Article Hub (SEO content layer)
An informational article hub drives top-of-funnel search discovery (each article targets long-tail queries and is a new entry point). It reuses the standard `.page-card` text-page template — flat URLs at `/[lang]/[slug].html`, same level as other pages.

- **Registry**: the `articles` array + `hubMeta` object near the top of `build_html.js`. Each article entry holds `slug`, `published`, `modified`, and per-language `title`/`description`, plus the optional `faq` flag (below).
- **Content**: `assets/docs/articles/[slug]_[lang].md` for EN/DE/RU. Article markdown starts with a single `# H1`. Follow the tone rules (§3.1) and use internal links (`guide.html`, `index.html#ls-analytics`, `cravings.html`, `articles.html`) plus the 🎁 gift blockquote for free features.
- **Hub page**: `articles.html` per language, generated by `buildArticleHub(lang)` (intro from `hubMeta.intro` + an auto-generated linked list of all articles). Reachable from the main nav ("Articles" / "Ratgeber" / "Статьи", i18n key `articles`).
- **SEO wiring**: `getPageTitleFull`/`getPageDescription` resolve articles + hub from the registry; `getSchemaOrg` emits `BlogPosting` + 3-level breadcrumb for articles and `Blog` + breadcrumb for the hub; `og:type` is `article` for article pages.
- **`faq: true` (optional registry flag)**: also emits `FAQPage` schema for that article, parsed by `parseArticleFaq()` from the article's **last `## ` section**. The convention there is `**Question?**` on one line with the answer on the following line(s), pairs separated by blank lines — the same shape the existing articles already use. Keep the question count identical across EN/DE/RU. Both this parser and the `faq` page branch share the `stripMd()` helper that flattens Markdown to the plain text schema.org expects.
- **`[[BAC_CALCULATOR]]` token**: article markdown may contain this token on its own line; the article write loop replaces the whole wrapping `<p>` with `buildBacCalculator(lang)` — the interactive blood-alcohol calculator (currently used by `promille-calculator`). Static labels are baked per language in `BAC_CALC_STRINGS` (so `translations.js` needs no entry); the few runtime strings ride along as `data-*` attributes. The maths lives in `initBacCalculator()` in `script.js` and is a **direct port of the app's `services/bac.ts`** (`c = A / (m × r)`, `r` = 0.7/0.6/0.65, 0.15 ‰/h, durations rounded **up** to 5 min, deliberately no absorption correction). **Keep the two in sync** — a visitor comparing the page against the app must see identical numbers. `llms-full.txt` substitutes a prose description for the token.
- **To add an article**: (1) create `assets/docs/articles/[slug]_[lang].md` (all 3 langs), (2) add an entry to the `articles` array, (3) add its 3 URLs + the hub lastmod to `sitemap-main.xml`, (4) add the article to the `## Articles` section of `llms.txt` (the static AI index — articles are deliberate AI/search entry points; `llms-full.txt` picks them up automatically from the `articles` array at build time), (5) `node build_html.js`.

**Current articles**: `promille-calculator` (blood alcohol / Widmark, DACH-focused, carries the live calculator + `faq: true`), `am-i-drinking-too-much`, `calories-in-alcohol`.
