...
- **Registry**: the `articles` array + `hubMeta` object near the top of `build_html.js`. Each article entry holds `slug`, `published`, `modified`, and per-language `title`/`description`.
